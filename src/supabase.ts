...
const createGitHubAccount = async (account: GitHubAccount) => {
...
const updateGitHubAccount = async (username: string, updates: Partial<GitHubAccount>) => {
  const { data, error } = await supabase
    .from('github_accounts')
    .update({
...
const migrateGitHubAccountsFromJson = async (jsonData: Record<string, any>) => {
  try {
